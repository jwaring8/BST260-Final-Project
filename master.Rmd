...
I think we should attempt to use `caret` for any and all machine learning models. It appears to have quite a few avaliable. 
...
First, let's train a random forest model on our training data, and then use the model to predict on the test set. 

```{r, cache=TRUE}
# train the model using randomForest
rf_model <- train(as.factor(readmitted) ~ ., data=trainData, method='parRF', tuneLength=1, trControl = trainControl(method="none"))
```
...
confusionMatrix(reference = as.factor(testData$readmitted), data = as.factor(predicted), mode='everything', positive='1')
```

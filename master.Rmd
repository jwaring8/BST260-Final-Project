...
GLMnet fits a generalized linear model via penalized maximum likelihood. The algorithm implemented in the package computes the regularization path for the elastic-net penalty over a grid of values for the regularization parameter $\lambda$ (https://web.stanford.edu/~hastie/glmnet/glmnet_alpha.html#log). To reduce noise caused by data-splitting, we used 10-fold cross-validation for tuning parameter values using the caret package as described here (http://www.milanor.net/blog/cross-validation-for-predictive-analytics-using-r/).
...
The most important features for predicting hospital readmission according to GLMnet are wheteher of not a patient was transfered to another facility at dischagre, the number of inpatient visists in the past year for a patient, and their A1C test result. 

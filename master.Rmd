...
data <- data[,!apply(cor_mat,2,function(x) any(abs(x) > 0.5))]
cor_mat2 <- cor(data[, 1:37])
...
dtrain <- xgb.DMatrix(data = as.matrix(trainData_SMOTE[, 1:37]), label = as.numeric(trainData_SMOTE$readmitted) - 1)
dtest <- xgb.DMatrix(data = as.matrix(testData[, 1:37]), label = as.numeric(testData$readmitted) - 1)
```
...
if(file.exists("xgb_params.csv")) {
  ...
  param_grid <- expand.grid(objective = "binary:logistic",
                          ...
                          max_depth = c(2,4,6),
                          gamma = c(2,3,4), 
                          scale_pos_weight = c(1.5, 2, 3))

  best_test_auc <- 0
  best_params <- param_grid[1, ]
   
  for (i in 1:nrow(param_grid)) {
    xgb_cv <- xgb.cv(params = param_grid[i, ], data = dtrain, nrounds = 300, 
...
```{r}
xgb <- xgb.train(params = best_params, data = dtrain, nrounds = 200, verbose = 0)
pred_xgb <- predict(xgb, dtest)
pred_xgb <- as.numeric(pred_xgb > 0.5)
confusionMatrix(reference = as.factor(testData$readmitted), data = as.factor(pred_xgb), mode = "everything", positive="1")
```
...
```{r}
pred <- prediction(pred_xgb, testData$readmitted)

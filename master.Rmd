...
GLMnet fits a generalized linear model via penalized maximum likelihood. The algorithm implemented in the package computes the regularization path for the elastic-net penalty over a grid of values for the regularization parameter λ (https://web.stanford.edu/~hastie/glmnet/glmnet_alpha.html#log). To reduce noise caused by data-splitting, we used 10-fold cross-validation for tuning parameter values using the caret package as described here (http://www.milanor.net/blog/cross-validation-for-predictive-analytics-using-r/).

#### Reading in the training and test dataset

The training dataset is the train_SMOTE dataset generated above.
...
#### Set up 10-fold data partition

```{r}
cv_splits <- createFolds(trainData$readmitted, k = 10, returnTrain = TRUE)
str(cv_splits)
```

#### Fit glmnet using elastic-net penalty using a range of alpha and lambdas
